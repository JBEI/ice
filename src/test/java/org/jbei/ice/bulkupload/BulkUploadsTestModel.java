 ...
 */
public class BulkUploadsTestModel extends HibernateRepositoryTest {

    private final BulkUploads uploads = new BulkUploads();

    @Test
    public void testCreate() throws Exception {
        AccountModel account = AccountCreator.createTestAccount("testBulkUploadCreate", false);
    ...
    public void testGetBulkImport() throws Exception {
        AccountModel account = AccountCreator.createTestAccount("testGetBulkImport", false);
    ...
    public void testRetrieveByUser() throws Exception {
        // create AccountModel 1. should not have any bulk uploads
        AccountModel account = AccountCreator.createTestAccount("testRetrieveByUser", false);
        ArrayList<BulkUpload> results = uploads.retrieveByUser("testRetrieveByUser", "testRetrieveByUser");
        Assert.assertEquals(0, results.size());

        // create strain with plasmid
        BulkUpload info = createUpload(account.getEmail(), EntryType.STRAIN);
        Assert.assertNotNull(info);
        ...
        // verify
        ArrayList<BulkUpload> userUpload = uploads.retrieveByUser(account.getEmail(), account.getEmail());
        Assert.assertNotNull(userUpload);
        Assert.assertEquals(1, userUpload.size());

        // create AccountModel 2
        AccountModel account2 = AccountCreator.createTestAccount("testRetrieveByUser2", false);
    ...
    private BulkUpload createUpload(String email, EntryType entryType) {
    ...
    public void testDeleteDraftById() throws Exception {
        AccountModel account = AccountCreator.createTestAccount("testDeleteDraftById", false);
        BulkUpload info = createUpload(account.getEmail(), EntryType.STRAIN);
    ...
    public void testAutoUpdateBulkUpload() throws Exception {
        AccountModel account = AccountCreator.createTestAccount("testAutoUpdateBulkUpload", false);
        BulkUpload info = createUpload(account.getEmail(), EntryType.STRAIN);
        ...
        // check bulk upload
        BulkUpload bulkUpload = uploads.get(account.getEmail(), bulkId, 0, 1000);
    ...
    public void testRevertSubmitted() throws Exception {
        // create accounts
        AccountModel account = AccountCreator.createTestAccount("testRevertSubmitted", false);
        AccountModel admin = AccountCreator.createTestAccount("testRevertSubmitted+Admin", true);
        BulkUpload info = createUpload(account.getEmail(), EntryType.SEED);
        ...
        BulkUpload retrieved = uploads.get(account.getEmail(), autoUpdate.getBulkUploadId(), 0, 0);
    ...
    public void testApproveBulkImport() throws Exception {
        AccountModel account = AccountCreator.createTestAccount("testApproveBulkImport", true);
        ...
        BulkUpload testInfo = new BulkUpload();
