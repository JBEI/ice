 ...
 */
public class BulkUploadModelEntriesTest extends HibernateRepositoryTest {
     ...
     */
    private BulkUploadModel createBulkUpload(String userId, EntryType entryType, EntryType linkType) {
    ...
    public void testCreateEntry() throws Exception {
        ...
        BulkUploadModel upload = createBulkUpload(account.getEmail(), EntryType.PLASMID, null);
    ...
    public void testUpdateEntry() throws Exception {
        ...
        // create bulk upload
        BulkUploadModel bulkUploadModel = createBulkUpload(account.getEmail(), EntryType.STRAIN, EntryType.PLASMID);
        Assert.assertEquals(bulkUploadModel.getStatus(), BulkUploadStatus.IN_PROGRESS);

        BulkUploadEntries entries = new BulkUploadEntries(account.getEmail(), bulkUploadModel.getId());
        ...
        // retrieve
        BulkUploads uploads = new BulkUploads();
        BulkUpload retrieved = uploads.get(account.getEmail(), bulkUploadModel.getId(), 0, 10);
        ...
        retrieved = uploads.get(account.getEmail(), bulkUploadModel.getId(), 0, 10);
        Assert.assertNotNull(retrieved);
        Assert.assertEquals(retrieved.getEntryList().size(), 1);
    }
    ...
    public void testUpdateStatus() throws Exception {
        ...
        BulkUploadModel bulkUploadModel = createBulkUpload(account.getEmail(), EntryType.PART, null);
        Assert.assertNotNull(bulkUploadModel);
        ...
        // verify entries
        bulkUploadModel = DAOFactory.getBulkUploadDAO().get(bulkUploadModel.getId());
        Assert.assertNotNull(bulkUploadModel);
        Assert.assertEquals(count, bulkUploadModel.getContents().size());

        for (Entry entry : bulkUploadModel.getContents()) {
        ...
        bulkUploadModel = DAOFactory.getBulkUploadDAO().get(bulkUploadModel.getId());
        Assert.assertNotNull(bulkUploadModel);
        Assert.assertEquals(BulkUploadStatus.PENDING_APPROVAL, bulkUploadModel.getStatus());
        ...
        entries = new BulkUploadEntries(admin.getEmail(), bulkUploadModel.getId());
        process = entries.updateStatus(BulkUploadStatus.APPROVED);
        Assert.assertTrue(process.isSuccess());

        // when done, bulk upload is deleted
        bulkUploadModel = DAOFactory.getBulkUploadDAO().get(bulkUploadModel.getId());
        Assert.assertNull(bulkUploadModel);
    }
    ...
    public void testCreateOrUpdateEntries() throws Exception {
        ...
        BulkUploadModel upload = createBulkUpload(account.getEmail(), EntryType.SEED, null);
        Assert.assertNotNull(upload);
        ...
        BulkUpload info = entries.createOrUpdateEntries(data);
