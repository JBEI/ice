...
import org.jbei.ice.entry.EntryPermissionTask;
...
public class PermissionsController {
    ...
    public Permission addPermission(String userId, AccessPermission access) {
        ...
        if (access.isFolder()) {
            ...
            if (!hasWritePermission(userId, folder)) {
                Logger.error(userId + " not allowed to add " + access);
                return null;
            }
    ...
    public boolean accountHasReadPermission(AccountModel account, Set<Folder> folders) {
        try {
    ...
    public boolean accountHasWritePermission(AccountModel account, Set<Folder> folders) {
        try {
    ...
    public List<AccessPermission> retrieveSetFolderPermission(Folder folder, boolean includePublic) {
        ...
        // read accounts
        List<AccountModel> readAccounts = dao.retrieveAccountPermissions(folder, false, true);
        for (AccountModel readAccount : readAccounts) {
        ...
        // write accounts
        List<AccountModel> writeAccounts = dao.retrieveAccountPermissions(folder, true, false);
        for (AccountModel writeAccount : writeAccounts) {
