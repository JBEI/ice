...
public class PlasmidViewPanel extends Panel {
	...
	public PlasmidViewPanel(String id, Plasmid entry) {
		...
		elements.add(new Label("originOfReplication", entry
				.getOriginOfReplication()));
		elements.add(new Label("creator", entry.getCreator()));
		elements.add(new Label("promoters", entry.getPromoters()));
		elements.add(new Label("status", org.jbei.ice.lib.utils.JbeiConstants
				.getStatus(entry.getStatus())));
		...
		elements
				.add(new Label("shortDescription", entry.getShortDescription()));
		...
		elements
				.add(new BookmarkablePageLink("updateLink",
						EntryUpdatePage.class, new PageParameters("0="
								+ entry.getId())));
