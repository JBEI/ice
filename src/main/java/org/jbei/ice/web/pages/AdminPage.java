...
public class AdminPage extends ProtectedPage {
    ...
    private void initializeControls(PageParameters parameters) {
    	...
    	// edit users tab
    	editUsersLink = new BookmarkablePageLink<Object>("editUsersLink", AdminPage.class, new PageParameters("0=users"));
    	editUsersLink.setOutputMarkupId(true);
    	
    	// edit parts tab
    	editPartsLink = new BookmarkablePageLink<Object>("editPartsLink", AdminPage.class, new PageParameters("0=parts"));
    	editPartsLink.setOutputMarkupId(true);
    	
    	// edit groups tab
    	editGroupsLink = new BookmarkablePageLink<Object>("editGroupsLink", AdminPage.class, new PageParameters("0=groups"));
    	...
    	editGroupsLink.add(new SimpleAttributeModifier("class", "inactive")).setOutputMarkupId(true);
    	...
    	if (currentPage !=null) {
    		if ("users".equals(currentPage)) {
    			...
    			editUsersLink.add(new SimpleAttributeModifier("class", "active")).setOutputMarkupId(true);
    		}
    		else if ("parts".equals(currentPage)) {
    			currentPanel = new EditPartsPanel(MAIN_PANEL_ID);
    			editPartsLink.add(new SimpleAttributeModifier("class", "active")).setOutputMarkupId(true);
    		}
    			...
    			editGroupsLink.add(new SimpleAttributeModifier("class", "active")).setOutputMarkupId(true);
    		}
			else {
