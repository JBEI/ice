...
public class SequenceCheckerPage extends WebPage {
    public SequenceCheckerPage(PageParameters parameters) {
        super(parameters);
    ...
    private void initialize(PageParameters parameters) {
        ...
        ResourceReference sctResourceReference = new ResourceReference(UnprotectedPage.class,
                UnprotectedPage.SCT_RESOURCE_LOCATION + "SequenceChecker.swf?projectId="
                        + projectId + "&sessionId=" + accountSessionId);

        flashComponent.add(new SimpleAttributeModifier("src", urlFor(sctResourceReference)));
