...
public class TestAttachmentManager {
	...
	public void testCreateGetDelete() throws IOException, ManagerException {
		...
		assertArrayEquals(createdAttachment1.getData().getBytes(), gotAttachment1.getData().getBytes());
		assertArrayEquals(createdAttachment2.getData().getBytes(), gotAttachment2.getData().getBytes());
	...
	public void testGetByEntryId() throws Exception {
		...
		try {
			ArrayList<Attachment> attachments = AttachmentManager.getByEntry(attachment1.getEntry());
