...
public class PartResource extends RestResource {
    ...
    @Path("/autocomplete")
    public ArrayList<String> autoComplete(@QueryParam("val") final String val,
            @DefaultValue("SELECTION_MARKERS") @QueryParam("field") final String field,
            @DefaultValue("8") @QueryParam("limit") final int limit) {
        final AutoCompleteField autoCompleteField = AutoCompleteField.valueOf(field.toUpperCase());
        final Set<String> result = retriever.getMatchingAutoCompleteField(autoCompleteField, val,
                limit);
    ...
    public ArrayList<PartData> autoComplete(@QueryParam("token") final String token,
            @DefaultValue("8") @QueryParam("limit") final int limit) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{id}/permissions")
    public ArrayList<AccessPermission> getPermissions(@Context final UriInfo info,
    ...
    public Response getPartExperiments(@PathParam("id") final long partId) {
        final String userId = getUserId();
        final ArrayList<Study> studies = experimentController.getPartStudies(userId, partId);
        if (studies == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{id}/comments")
    public ArrayList<UserComment> getComments(@Context final UriInfo info,
            @PathParam("id") final long partId) {
        final String userId = getUserId();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{id}/attachments")
    public ArrayList<AttachmentInfo> getAttachments(@PathParam("id") final long partId) {
        final String userId = getUserId();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{id}/samples")
    public ArrayList<PartSample> getSamples(@Context final UriInfo info,
            @PathParam("id") final long partId) {
        final String userId = getUserId();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{id}/samples")
    public ArrayList<PartSample> addSample(@Context final UriInfo info,
            @PathParam("id") final long partId,
    ...
    public Response getSequence(@PathParam("id") final long partId,
            @QueryParam("sid") final String sessionId) {
        final String userId = getUserId(sessionId);
        final FeaturedDNASequence sequence = sequenceController
                .retrievePartSequence(userId, partId);
        if (sequence == null) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public PartData create(@Context final UriInfo info, final PartData partData) {
        final String userId = getUserId();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response transfer(final PartData partData) {
        final EntryCreator creator = new EntryCreator();
        final PartData response = creator.receiveTransferredEntry(partData);
