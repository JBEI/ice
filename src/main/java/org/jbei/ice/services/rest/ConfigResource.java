...
public class ConfigResource extends RestResource {
    ...
    @PUT
    @Path("/lucene")
    public void buildLuceneIndex(@HeaderParam(value = "X-ICE-Authentication-SessionId") String userAgentHeader) {
            String userId = getUserIdFromSessionHeader(userAgentHeader);
            Account account = DAOFactory.getAccountDAO().getByEmail(userId);
            if (account == null)
                return;

            new SearchController().rebuildIndexes(account, IndexType.LUCENE);
    }
