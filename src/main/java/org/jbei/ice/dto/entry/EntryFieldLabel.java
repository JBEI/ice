...
public enum EntryFieldLabel implements IDataTransferModel {
    ...
    EXTERNAL_URL("External URL", false, FieldInputType.LONG_TEXT),
    ...
    SELECTION_MARKERS("Selection Markers", true, FieldInputType.LONG_TEXT),
    ...
    public static EntryFieldLabel fromLabel(EntryType type, String labelString) {
        for (EntryFieldLabel label : EntryFieldLabel.values()) {
            if (labelString.equalsIgnoreCase(label.getDisplay()))
                return label;
        }
        return null;
    }
